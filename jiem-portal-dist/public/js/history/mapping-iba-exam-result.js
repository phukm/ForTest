var sessionNameMapping = 'sessionStorageIbaResult';
var keyName = 'iba';
var MAPPING_EXAM_RESULT = {
    ajaxGetListClass: COMMON.baseUrl + 'history/eiken/ajax-get-list-class',
    confirmStatus: COMMON.baseUrl + 'history/iba/confirm-status',
    studentList: COMMON.baseUrl + 'pupil/pupil/index',
    actionDetails: COMMON.baseUrl + 'history/iba/iba-confirm-result',
    actionGetStudents: COMMON.baseUrl + 'history/eiken/get-students',
    actionCallSaveNextPupil: COMMON.baseUrl + 'history/iba/call-save-next-pupil',
    actionEikenMappingResult: COMMON.baseUrl + 'history/iba/iba-mapping-result',
    actionSearch: COMMON.baseUrl + 'history/iba/search',
    documentReferrer: COMMON.baseUrl + 'history/iba/',
    documentReferrerExamResult: COMMON.baseUrl + 'history/eiken/exam-result',
    initital: function () {
        var token = $('#token').val();
        var year = $("#year").val();
        var jisshiId = $("#jisshiId").val();
        ...
        var count = 0;
        var curentIndex = 0;
        var ibaTestResultId = $('#id').val();

        if (typeof eikenTestResultId != 'undefined' && eikenTestResultId != '' && !listIdOld[keyName + ibaTestResultId]) {
            window.location.href = MAPPING_EXAM_RESULT.actionEikenMappingResult + '/year/' + year + '/jisshiId/' + jisshiId + '/examType/' + examType;
            return false;
        }
        if (typeof listIdOld != 'undefined' && !$.isEmptyObject(listIdOld) && listIdOld != null) {
            $.each(Object.keys(listIdOld), function (index, element) {
                if (element == keyName + ibaTestResultId) {
                    curentIndex = index;
                }
                count = index;
            });
    ...
    callSaveData: function (ibaTestResultId, pupilId, type) {
        var year = $("#year").val();
        var jisshiId = $("#jisshiId").val();
        var examType = $("#examType").val();
        ...
        $.ajax({
            ...
            data: {ibaTestResultId: ibaTestResultId, pupilId: pupilId, type: type, typeMapping: typeMapping},
            dataType: "json",
            success: function (data) {
                ERROR_MESSAGE.show(messages.eikenResultConfirmSuccessMEG58, function () {
                    if (data.type == 'next') {
