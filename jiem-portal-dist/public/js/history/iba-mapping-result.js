var IBA_MAPPING_RESULT = {
        ...
        linkMapping: COMMON.baseUrl + "history/iba/mapping-result",
        linkKeepSession: COMMON.baseUrl + "history/iba/find-iba",
        saveMapping: COMMON.baseUrl + "history/iba/save-maping-iba",
        cancelMapping: COMMON.baseUrl + "history/iba/clear-session",
        popupOpened: false,
		init: function(){
			IBA_MAPPING_RESULT.autoFocus();
			IBA_MAPPING_RESULT.checkExists==true;
			IBA_MAPPING_RESULT.checkIE();
			$(document).on('click','.nav-confirm-result li',function(){
				IBA_MAPPING_RESULT.autoFocus($(this).find('a').attr("href"));
			});
			$(window).scroll(function(){
				 if(IBA_MAPPING_RESULT.checkIE8==true){
		                var height =  IBA_MAPPING_RESULT.elementTable.clientHeight;
		                var top = IBA_MAPPING_RESULT.elementTable.offsetTop - document.documentElement.scrollTop;
		                var bottom = document.documentElement.scrollTop - IBA_MAPPING_RESULT.elementTable.offsetTop;
						...
						var demension = IBA_MAPPING_RESULT.getDimensions(IBA_MAPPING_RESULT.elementTable);
			...
			if($('.mapping-error-autocomplete').length){
				$(".mapping-error-autocomplete" ).autocomplete({
			      source: function(req, response) {
              	    var results = $.ui.autocomplete.filter(IBA_MAPPING_RESULT.states, req.term);
			      ...
			      select: function (event, ui) {
                      ...
                      var data = IBA_MAPPING_RESULT.checkSelectedData($(elementFocus),ui.item);
                      
			    	  if(IBA_MAPPING_RESULT.checkSltData == true){
			    		  IBA_MAPPING_RESULT.autoFill(elementFollow, elementFocus, ui.item);
			    		  IBA_MAPPING_RESULT.ajaxKeepSession($(this).data('id'),ui.item);
			    		  
			    	  } else {
			    		  IBA_MAPPING_RESULT.showMSG18(data,elementFollow, elementFocus, ui.item, $this.data('id'));
			    	  }
			    	  
			    	  return false;
			      },
			      change: IBA_MAPPING_RESULT.onAutocompleteChange,
                  close: IBA_MAPPING_RESULT.onAutocompleteChange
			 });
			}
            ...
            $('#table-right-success tbody tr').each(function(index){ 
                $('#table-left-success tbody').find('tr').eq(index).css('height', $(this).height() + 'px');
			});
		...
		onAutocompleteChange: function(event, ui){
            ...
            if($this.is(":focus")==false){
                if(IBA_MAPPING_RESULT.popupOpened) return;
                IBA_MAPPING_RESULT.popupOpened = true;
                if(IBA_MAPPING_RESULT.checkBlank($this)==true){
                    // blank - You must input a name.
                    ERROR_MESSAGE.popupMessage('氏名を入力してください。',
                            function(){
                                $this.val($this.attr('data-value'));
                                IBA_MAPPING_RESULT.popupOpened = false;
                            }
                            ,'エラーメッセージ');
                    return;
                }
                IBA_MAPPING_RESULT.checkNameExists($this.val());
                if(IBA_MAPPING_RESULT.checkExists==false){ 
                ...
                IBA_MAPPING_RESULT.setNameExists(true);
            }
        },
		autoFill: function(elementFollow, elementFocus, item){
			$(elementFocus+' .jname input').attr('data-value', item.firstNameKana + item.lastNameKana);
        	$(elementFocus+' .jname input').attr('value', item.firstNameKana + item.lastNameKana);
		...
		checkIE:function(){
			if(document.addEventListener){
				IBA_MAPPING_RESULT.checkIE8 = false;
	        }else{
	        	IBA_MAPPING_RESULT.checkIE8 = true;
	        }

		},
		setNameExists: function(value){
			IBA_MAPPING_RESULT.checkExists= value;
		},
		checkNameExists: function(value){
			var result = false;
			jQuery.grep(IBA_MAPPING_RESULT.states, function(el) {
				if(value==el.value) result = true;
			});
			IBA_MAPPING_RESULT.setNameExists(result);
		},
		...
		mappingSave: function(){
			IBA_MAPPING_RESULT.ajaxSaveMapping();
		},
		mappingCancel: function(){
			$.ajax({
				type:"POST",
				url: IBA_MAPPING_RESULT.cancelMapping,
				dataType : 'html',
				success:function(data){
					window.location.replace(IBA_MAPPING_RESULT.linkBack);
					return true;
				},
				error:function(){
					return false;
		...
		showMSG18: function(data, elementFollow, elementFocus, item, ibaId){
            var msg = IBA_MAPPING_RESULT.convertMsg18(data);
            IBA_MAPPING_RESULT.popupOpened = true;
            CONFIRM_MESSAGE.show(msg, function(){
            								IBA_MAPPING_RESULT.autoFill(elementFollow, elementFocus, item);
            								IBA_MAPPING_RESULT.ajaxKeepSession(ibaId ,item);
            								IBA_MAPPING_RESULT.popupOpened = false;
                                        }, function(){
		...
		// no exists - No pupils found.
		showMSG20: function(){
			ERROR_MESSAGE.popupMessage('生徒が見つかりませんでした。',null,'エラーメッセージ');
		},
		// blank - You must input a name.
		showMSG21: function(){
			ERROR_MESSAGE.popupMessage('氏名を入力してください。',null,'エラーメッセージ');
		},
		convertMsg18: function(data) {
			//var msgQuestion = '選択された生徒の情報は以下の通りとなります。突合せを行ないますか？';
			var msg = '';// =;
			msg += '<ul>';
			//check empty
		...
		autoFocus: function(e){
			...
			if($('#table-left').length){
				IBA_MAPPING_RESULT.elementTable = $('#table-left')[0];
			}else {
				IBA_MAPPING_RESULT.elementTable = $('#table-left-success')[0];
			}
		...
		progress: function(){
			...
			if ( val < 100 ) {
				...
				//Redirect to UC
				window.location.href= '/history/eiken/mapping-result';
			}
			
		},
		ajaxKeepSession: function(ibaId,item){
			var data = {};
			if(item.birthday!==null){
			...
			$.ajax({
				type:"POST",
				url: IBA_MAPPING_RESULT.linkKeepSession,
				data:data,
				dataType : 'html',
				success:function(data){
					return;
		...
		ajaxSaveMapping: function(){
			$.ajax({
				type:"POST",
				url: IBA_MAPPING_RESULT.saveMapping,
				dataType : 'html',
				success:function(data){
					window.location.replace(IBA_MAPPING_RESULT.linkBack);
					return true;
		...
		checkSelectedData:function(apiData,pupilData){
                    ...
                    if(apiData.children('.jname').children('input').data('value') !== ($.trim(pupilData.firstNameKanji)+$.trim(pupilData.lastNameKanji))){
                        IBA_MAPPING_RESULT.checkSltData = false;
                    } 
                    data.push({id:"NameKanji",message:'氏名(漢字): '+ COMMON.escapeHtml($.trim(pupilData.firstNameKanji) + $.trim(pupilData.lastNameKanji))});
                    //check Kantana
                    if($.trim(apiData.children('.jname').children('input').attr('data-value')) !== ($.trim(pupilData.lastNameKana)+$.trim(pupilData.firstNameKana))){
                        IBA_MAPPING_RESULT.checkSltData = false;
                    }
                    var strNameKana = '';
                    if(pupilData.nameKantana !== null){
                    ...
                    var strSchoolYear = '';
                    if(pupilData.orgSchoolYearName !== null){
