...
class UpdateConfirmStatusForEikenResultTest extends \Dantai\Test\AbstractHttpControllerTestCase{
    public function testUpdateConfirmStatusForEikenResult()
    {
        $this->login();
        ...
        $a->eikenArray[0]->nendo = "2016";
        $a->eikenArray[0]->kai = "1";
        $a->eikenArray[0]->dantaino = "99966696";
        ...
        $result = $mappingEikenResult->saveEikenExamResult($a, "99966696", 18, 1, 2016, 1);
        
        $recordEikenResult = $this->getEntityManager()
                ->getRepository('Application\Entity\EikenTestResult')
                ->findOneBy(array('nameRomanji' => 'EIKENRESULTTEST'));
        
        $recordId = $recordEikenResult ? $recordEikenResult->getId() : NULL;
        $result = array(
            'listIdOld' => array(
                $recordId => $recordId
            ),
        ...
        $this->dispatch('/history/eiken/confirm-status', \Zend\Http\Request::METHOD_POST, $result);
        $finalResult = $this->getResponse()->getContent();
        $this->assertNotNull($finalResult);
    ...
    {
        $testResult =  $this->getEntityManager()->getRepository('Application\Entity\EikenTestResult')
                ->findOneBy(array('organizationNo' => '99966696', 'nameRomanji' => 'EIKENRESULTTEST'));
        $this->getEntityManager()->remove($testResult);
