...
var CONFIRM_PAYMENT_METHOD_MESSAGE = {
	show: function (msg, fncMethod1CallBack, fncMethod2CallBack, fncKOCallBack, title, nameMethod1, nameMethod2, nameCancel) {
		if (typeof title === 'undefined' || title === null)
			title = "確認";		
		if (typeof nameCancel === 'undefined' || nameCancel === null)
			nameCancel = "キャンセル";
		...
		confirmPopup += '<div class="modal-footer">';
		//confirmPopup += '<button class="btn btn-large-180" id="btnCancelModel">キャンセル</button>';		
                confirmPopup += '<button class="btn btn-large-180" id="btnCancelConfirm">';
		confirmPopup += nameCancel;
		confirmPopup += '</button>';	
		if (nameMethod1 != null){
                    confirmPopup += '<button class="btn btn-large-180 btn-red" id="btnMethod1Confirm" >';
                    confirmPopup += nameMethod1;
                    confirmPopup += '</button>';
                }
		if (nameMethod2 != null){
                    confirmPopup += '<button class="btn btn-large-180 btn-red" id="btnMethod2Confirm" >';
                    confirmPopup += nameMethod2;
                    confirmPopup += '</button>';   
                }
		confirmPopup += '</div>';
		confirmPopup += '</div>';
		...
		$('body').append(confirmPopup);
		$('#confirmPopupModal').modal('show');
		$('#btnMethod1Confirm').bind('click', function () {
			if (typeof fncMethod1CallBack === 'function') {
				fncMethod1CallBack();
			}
			$('#confirmPopupModal').modal('hide');
		});
                $('#btnMethod2Confirm').bind('click', function () {
			if (typeof fncMethod2CallBack === 'function') {
				fncMethod2CallBack();
			}
			$('#confirmPopupModal').modal('hide');
		});
